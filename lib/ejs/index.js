...
var parse = exports.parse = function(str, options){
    ...
    for (var i = 0, len = str.length; i < len; ++i) {
        if (str.slice(i, open.length + i) == open) {
            ...
            var js, rawCode = str.substring(start, end);
            if (rawCode[0] == ':') js = filtered(rawCode);
            else js = prepareJsCode(rawCode, c, jsInputMode, closeHeap);
...
function prepareJsCode(rawCode, extraCode, jsInputMode, closeHeap) {
    var jsCode = rawCode,
        er,
