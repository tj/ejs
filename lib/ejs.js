...
var parse = exports.parse = function(str, options){
  ...
  if (scope.type === EXTEND) {
    var path = scope.path;
    scope = stack.pop();
    scope.buf.push(endextend(path, { filename: path, _with: false, open: open, close: close, compileDebug: compileDebug, _blocks: false }));
  }
  if (false !== options._with) scope.buf.push("'); })();\n} \nreturn buf.join('');");
  else scope.buf.push("');\nreturn buf.join('');");
...
function commands(js, stack, channels, options) {
  ...
  switch(true) {
    case /^extend\s/.test(command):
      ...
      scope = stack.push({
        type: EXTEND,
        path: resolveFile(name, options.filename),
        buf: channels[HIDDEN]
      });
      return '';
    case /^block\s/.test(command):
      if (scope.type === BLOCK) throw new Error('expecting endblock, block found');
      scope = stack.push({
        type: BLOCK,
        name: js.trim().slice(5).trim(),
        buf: channels[MAIN]
      });
      scope.buf.push(block(scope.name));
      return '';
